...
haystack = Image.open("missing_frames/missing_052428_022.jpg").convert("RGB")
template = Image.open("candidate_text_crop2.png").convert("RGB")

h = cv2.cvtColor(np.array(haystack), cv2.COLOR_RGB2BGR)
t = cv2.cvtColor(np.array(template), cv2.COLOR_RGB2BGR)

print(f"Haystack: {h.shape}, Template: {t.shape}")
...
cv2.imwrite("debug_current_match.jpg", vis)
print("Saved debug_current_match.jpg")
